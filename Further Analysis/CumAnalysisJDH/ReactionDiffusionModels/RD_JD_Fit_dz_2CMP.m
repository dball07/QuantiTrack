function [FitPar, FitParSigma, FitSSR, Fit] = RD_JD_Fit_dz_2CMP(Data, Par, Ini, parFlag, plotFlag)
...
% Par{5} = deltaZ
% Par{6} = [D1,D2], diffusion coefficients for the two components
%
...
if parFlag == 0
    options = optimset('FunValCheck','off','UseParallel',false);
else
    options = optimset('FunValCheck','off','UseParallel',true);
end
...
if plotFlag ~= 0
 ...
 figure;
    ...
    title({'Jump Histogram Distribution - Full Model Fit',...
        ['D1 = ', num2str(Par{6}(1),3), '\mum^2/s, D2 = ', ...
        num2str(Par{6}(2),3), '\mum^2/s'],...
...
disp(['D1 = ', num2str(Par{6}(1),2), '\mum^2/s'])
disp(['D2 = ' num2str(Par{6}(2),2), '\mum^2/s'])
disp(['kon = ', num2str(FitPar(3),2), 's^{-1}'])
disp(['koff = ', num2str(FitPar(4),2), 's^{-1}'])
disp(['deltaZ = ', num2str(Par{5},2), '\mum'])
disp('_________________________________________')
