% Modified by KW to save fig files in addition to pdfs

function [Output_struct] = SpotOn_core_vKW_QT(Params)
        ...
        for WorkIter=1:length(workspaces)
            ...
            if ceil(CellNumb/8) > last_integer
                ...
                if SavePlot == 1 && PlotIndex > 1
                    ...
                    savefig(figure1,[save_path, SampleName, '_SingleCell_FitType',num2str(ModelFit), 'FitLocError', num2str(FitLocError), 'NumStates', num2str(NumberOfStates), 'Plot', num2str(PlotNumber-1),'.fig']);
                    exportgraphics(figure1,[save_path, SampleName, '_SingleCell_FitType',num2str(ModelFit), 'FitLocError', num2str(FitLocError), 'NumStates', num2str(NumberOfStates), 'Plot', num2str(PlotNumber-1),'.eps']);
                    close(figure1);
                end
            ...
            xlabel('jump length \mum', 'FontSize',9, 'FontName', 'Helvetica', 'Color', 'k');
...
[model_params, ci, residuals, list_of_model_parameters] = QT_ModelFitting_main(JumpProb, JumpProbCDF, NumberOfStates, FitLocErrorRange, FitIterations, D_Free_2State, D_Bound_2State, D_Free1_3State, D_Free2_3State, D_Bound_3State);
toc;
...
if DoPlots == 1 % proceed to plotting only if DoPlots == 1
    ...
    if SavePlot == 1
        ...
        savefig(figure2,[save_path, SampleName, '_TrajLengthDist.fig']);
        exportgraphics(figure2,[save_path, SampleName, '_TrajLengthDist.eps']);
        close(figure2);
    end
    ...
    for i=1:min([12 size(residuals,1)])
        ...
        xlabel('displacements (\mum)', 'FontSize',9, 'FontName', 'Helvetica', 'Color', 'k');
        hold off;
    end
    ...
    if SavePlot == 1
        ...
        savefig(figure3,[save_path, SampleName, '_residuals_FitType',num2str(ModelFit), 'NumStates', num2str(NumberOfStates), '.fig']);
        exportgraphics(figure3,[save_path, SampleName, '_residuals_FitType',num2str(ModelFit), 'NumStates', num2str(NumberOfStates), '.eps']);
        close(figure3);
    end
    ...
    if SavePlot == 1
        ...
        savefig(figure4,[save_path, SampleName, '_mergedCDFs_FitType',num2str(ModelFit), 'NumStates', num2str(NumberOfStates), '.fig']);
        exportgraphics(figure4,[save_path, SampleName, '_mergedCDFs_FitType',num2str(ModelFit), 'NumStates', num2str(NumberOfStates), '.eps']);
        close(figure4);
    end
    ...
    if SavePlot == 1
        ...
        savefig(figure5,[save_path, SampleName, '_mergedPDFs_FitType',num2str(ModelFit), 'FitLocError', num2str(FitLocError), 'NumStates', num2str(NumberOfStates), '.fig']);
        exportgraphics(figure5,[save_path, SampleName, '_mergedPDFs_FitType',num2str(ModelFit), 'FitLocError', num2str(FitLocError), 'NumStates', num2str(NumberOfStates), '.eps']);
        close(figure5);
    end 
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Output_struct(1).merged_model_params_ci = ci;
