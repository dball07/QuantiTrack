function [best_vals, best_ci, residuals, list_of_model_parameters] = QT_ModelFitting_main(JumpProb, JumpProbCDF, NumberOfStates, FitLocErrorRange, FitIterations, D_Free_2State, D_Bound_2State, D_Free1_3State, D_Free2_3State, D_Bound_3State)
...
%%%%%%%%%%%%%%%%%   KW update 2025/01/13   %%%%%%%%%%%%%%%%%%%%%
% Added calculation of fit parameter confidence intervals from the Jacobian
...
for iter=1:FitIterations
    ...
    if NumberOfStates == 2 && FitLocError == 0 % 2-state model, fixed Loc Error
        ...
        [values, ssq2, residuals, ~, ~, ~, J] = lsqcurvefit('Model_2State', parameter_guess, ModelHistVecJumps, FitJumpProb, LB, UB, options);

    elseif NumberOfStates == 2 && FitLocError == 1 % 2-state model, Loc Error from fitting
        [values, ssq2, residuals, ~, ~, ~, J] = lsqcurvefit('Model_2State_fitLocError', parameter_guess, ModelHistVecJumps, FitJumpProb, LB, UB, options);

    elseif NumberOfStates == 3 && FitLocError == 0 % 3-state model, fixed Loc Error
        [values, ssq2, residuals, ~, ~, ~, J] = lsqcurvefit('Model_3State', parameter_guess, ModelHistVecJumps, FitJumpProb, LB, UB, options);

    elseif NumberOfStates == 3 && FitLocError == 1 % 3-state model, Loc Error from fitting
        [values, ssq2, residuals, ~, ~, ~, J] = lsqcurvefit('Model_3State_fitLocError', parameter_guess, ModelHistVecJumps, FitJumpProb, LB, UB, options);
    end

    ci  = QTcalculate_ci_from_jacobian(J, values, residuals, ssq2, 0.05);
