function [D, Rc,D_CI, Rc_CI, D_fit, Rc_fit] = fitMSD_Rc(lags,msd,varargin)
...
fitfun = @(COEF) (Confine_fun(COEF, dataR(:,1) ,D) -  dataR(:,2));
...
% Compute output
Rc_fit(:,1) = dataR(:,1);
Rc_fit(:,2)= Confine_fun(Rc,Rc_fit(:,1),D);

function MSD = Confine_fun(Rc,t,D)

MSD = (Rc(2).^2)*(Rc(1).^2)*(1 - exp(-4*D*t/(Rc(1).^2)))  ;
