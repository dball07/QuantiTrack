function CentroidInAllRois = InsideROIcheck2_ReTrack(Centroid, ROIimage,varargin) 
...
% end
%  BG = BG == 0;   
ROI_image_all = cell(size(ROIimage,1),1);
for i = 1:size(ROIimage,1)
    ROI_image_all{i,1} = ROIimage{i,1};
    for j = 2:size(ROIimage,2)
        ROI_image_all{i,1} = ROI_image_all{i,1} + ROIimage{i,j};
    end
    ROI_image_all{i,1} = ROI_image_all{i,1} > 0;
    
end

for ROIindx = 1:size(ROIimage,1)
    ROI_image_cur = ROIimage(ROIindx,:);
    isInside = zeros(NParticles,1);
    for ROIindx2 = 1:size(ROIimage,1)
        for imageInd = 1:size(ROIimage,2)
            if ROI_nest_mat(ROIindx,ROIindx2) == 1
                ROI_image_cur{:,imageInd} = ROI_image_cur{:,imageInd} - ROIimage{ROIindx2};
                ROI_image_cur{:,imageInd} = ROI_image_cur{:,imageInd} > 0;
            end
...
%     ROI_image_cur(BG) = 0;
    ...
    for particleIx = 1:NParticles
        ...
        if size(ROI_image_cur,2) == 1
            particlePos(:,frameInd) = 1;
        end
        if particlePos(xspec) > 0
            if ROI_image_cur{:,particlePos(frameInd)}(max(1,round(particlePos(yspec))),max(1,round(particlePos(xspec)))) == 1
    ...
    if size(Centroid,2) > 5 && zero_flag == 1
        for i = 1: max(CentroidInRoi{ROIindx}(:,6))
            Index = find(CentroidInRoi{ROIindx}(:,6) == i,1);
            ...
            if isempty(Index)
                ...
                if size(Centroid,2) == 18
                    addvec = [addvec zeros(1,5)];
                end
