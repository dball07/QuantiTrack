 function [output] = forward_filter2D( img_4d,options )
...
for i = 1:n_t
    ...
    candidates = find_candidates2D(img_3d,means,weights,options);
    % set gate around last value and get roi
    [z,roi] = get_data_advanced2D(img_3d,candidates,options.boxsize);
    % perform indivdual updates for all possible transitions
    [means,covariances,weights,max_log_weight] = forward_update2D(means,covariances,...
