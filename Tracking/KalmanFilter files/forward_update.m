function [ means,covariances,weights,max_log_weight ] = forward_update( means_old,covariances_old,...
...
[R_basis,R] = construct_noise_matrix(roi,options);
...
% transition off -> off
    % perform prediction
    [x_pre,P_pre] = mixture_reduction(prior_weights,{x_pre_1,x_pre_2},{P_pre_1,P_pre_2});
    x_pre(5) = 0;
    P_pre = P_pre+Q;
    P_pre(:,5) = 0;
    P_pre(5,:) = 0;
    ...
    fun = @(x) 0.5*sum((R_basis'*(z-x(4))).^2./R)+0.5*(x-x_pre)'*(P_pre\(x-x_pre));
%     % perform fit
%     x_start = [mean(roi)';x_pre(4);0.1*std(z)];
%     [x,res,~,~,~,H] = fminunc(fun,x_start,options.fit_options);
    [x,H] = optimize_background(x_pre,P_pre,z,R_basis,R);
    res = fun(x);
...
% transition off -> on
    ...
    fun = @(x) extended_observation_model_grad(x,roi,z,R_basis,R,options.psf_parameters,x_pre,P_pre);
    ...
    for j = 1:length(candidates)
        ...
        x_start(1:3) = candidates{j}(1:3);
        x_start(4) = means{off,off}(4);
        x_start(5) = prefit_intensity(x_start,roi,z,R_basis,R,options.psf_parameters,x_pre,P_pre);
        % perform fit with all candidates as initial points
        [x_test,res_test,~,~,~,H_test] = fminunc(fun,x_start,options.fit_options);
        if is_valid_cov(H_test) && res_test < res && is_valid_state(x_test,options)
            x = x_test;
            res = res_test;
            H = H_test;
        end
