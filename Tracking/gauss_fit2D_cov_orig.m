function  [parFit, ssr, parSigma] = gauss_fit2D_cov_orig(img,coordinates)
...
[parFit,resNorm, residuals,~,~,~,jacobian] = lsqnonlin(@(P)objfun(P,px,py,img),par0,LB,UB,options);
ci = nlparci(parFit,residuals,'jacobian',jacobian);
parSigma = (ci(:,2) - ci(:,1))/2;
parSigma = parSigma';
