function [peak_summary_new] = peak_fit_psf(stack,peak_summary,dx,dy,useParallel)
...
if useParallel == 1
    fwait = waitbar(1,'Fitting Particle PSFs in Parallel','Name','Finding Particles');
    parfor iFrame = 1:nFrames
        ...
        if nPeaks > 0
            ...
            PeaksInCurrFrame = [PeaksInCurrFrame(:,1:7), zeros(nPeaks,11)];
            ...
            if PeaksInCurrFrame(1,1) == 0 && PeaksInCurrFrame(1,2) == 0
                ...
                for iPeak = 1:nPeaks
                    ...
                    if x_center ~= 0
                        ...
                        [parFit, ssr, parSigma] = gauss_fit2D_cov_orig(img_sub,coordinates);
                        
                        
                        % Add fit parameters to the output
                        PeaksInCurrFrame(iPeak, 8:18) = [parFit, ssr, parSigma];
    ...
    fwait = waitbar(0,['Fitting Particle PSFs in Frame 1 of ', num2str(nFrames)],'Name','Finding Particles');
    for iFrame = 1:nFrames
        ...
        if nPeaks > 0
         ...
         waitbar(iFrame/nFrames,fwait,['Detecting Particles in Frame ', num2str(iFrame+1), ' of ', num2str(nFrames)],'Name','Finding Particles');
    end
    waitbar(1,fwait,['Detecting Particles in Frame ', num2str(iFrame+1), ' of ', num2str(nFrames)],'Name','Finding Particles');
    close(fwait);
end
%move ROI data to last column to preserve downstream processing
if ~isempty(peak_summary_new)
    peak_summary_new = [peak_summary_new(:,1:6) peak_summary_new(:,8:13) peak_summary_new(:,7) peak_summary_new(:,14:18) ];
