...
delete(findobj(ax_h,'MarkerSize',12));
...
for i = 1:max(Particles(:,ROIindx))
    ...
    if ~isempty(pIx)
        hold(ax_h,'on');
        if isPSF
            plot (ax_h,Particles_tmp(pIx,10),Particles_tmp(pIx,11),'o', 'MarkerSize', 12,'MarkerEdgeColor',origCO(i,:));
        else
            plot (ax_h,Particles_tmp(pIx,1),Particles_tmp(pIx,2),'o', 'MarkerSize', 12,'MarkerEdgeColor',origCO(i,:));
        end
        hold(ax_h,'off');
    end
end
