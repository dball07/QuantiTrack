function ROI_nest_matrix = roiNest(ROIimage)
...
nROIs = size(ROIimage,1);
ROI_nest_matrix = zeros(nROIs);

ROI_image = cell(nROIs,1);
for i = 1:size(ROIimage,1)
    ROI_image{i,1} = ROIimage{i,1};
    for j = 2:size(ROIimage,2)
        ROI_image{i,1} = ROI_image{i,1} + ROIimage{i,j};
    end
    ROI_image{i,1} = ROI_image{i,1} > 0;
